...
def MakeModel(model):
    ...
    # '------- CreateCylinder -------
    Point2 = Dispatch(const.CGaudiPoint)  # CGaudiPointクラスのインスタンス作成
    Point2.SetCoord(model["rad"], 0, -(model["thick"]))
    tmpBody = Gaudi.CreateCylinder(Point2, model["r"], model["thick"])
    BodyList.append(tmpBody)  # リストに追加
    ...
    ret = BodyList[1].RingCopy_py(Point3, Vector0, 60, 5)
    BodyList.extend(ret[1])
    ret = BodyList[2].RingCopy_py(Point3, Vector0, 60, 5)
    BodyList.extend(ret[1])
    BodyList2 = [BodyList[2]]
